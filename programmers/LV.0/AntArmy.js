...
function solution(hp) {
  ...
  let restHp = hp;
  if (restHp >= 5) {
    let { quotient, remainder } = quotientAndRemainder(restHp, 5);
    answer += quotient;
    restHp = remainder;
  }

  if (restHp >= 3 && restHp > 1) {
    let { quotient, remainder } = quotientAndRemainder(restHp, 3);
    answer += quotient;
    restHp = remainder;
  }
...
function quotientAndRemainder(hp, counter) {
  let quotient = Math.floor(hp / counter);
  let remainder = hp % counter;
